...
%             	addpath(fullfile('Algorithms','libsvm-rank-2.81','matlab'));
             addpath('/root/NPSVOR_matlab/Algorithm/libsvm-rank-2.81/matlab')
                param.C = parameters(1);
                param.k = parameters(2);
                train.UniqueLabel = sort(unique(train.targets))';
                test.UniqueLabel = train.UniqueLabel;
                train.targets0 = train.targets;
                for i = 1:length(train.UniqueLabel)
                    train.targets0(train.targets0==train.UniqueLabel(i)) = i;
                end
                c1 = clock;
...
%             	rmpath(fullfile('Algorithms','libsvm-rank-2.81','matlab'));
            rmpath('/root/NPSVOR_matlab/Algorithm/libsvm-rank-2.81/matlab')
        ...
        function [model]= train( obj, train , param)
            ...
            model = svmtrain0(train.targets, train.patterns, options);
  %           model.UniqueLabel = train.UniqueLabel;
        end
        ...
        function [projected, testTargets]= test(obj,test, model)
                ...
                [testTargets, acc, projected] = svmpredict0(test.targets,test.patterns,model, '');
                 testTargets = test.UniqueLabel(testTargets)';
        end      
    end
end
